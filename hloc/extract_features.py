...
def main(conf: Dict,
    ...
    if model is None:
        print('Model is None, load model')
        time1 = time.time()
        Model = dynamic_load(extractors, conf['model']['name'])
        time2 = time.time()
        print(' dynamic_load for ', time2-time1, 's')
        model = Model(conf['model']).eval().to(device)
        time1 = time.time()
        print(' create model for ', time1-time2, 's')
    else:
        print('model loaded, start prefering')
    time1 = time.time()

    loader = torch.utils.data.DataLoader(
        dataset, num_workers=1, shuffle=False, pin_memory=True)
    time3 = time.time()
    # print(' load data for ', time3-time1, 's')
    for idx, data in enumerate(tqdm(loader, smoothing=.1)):
        ...
        time4 = time.time()
        # print(' net working for ', time4-time3, 's')
        with h5py.File(str(feature_path), 'a', libver='latest') as fd:
            try:
                ...
                raise error
        time5 = time.time()
        # print(' saving features for ', time5-time4, 's')
        del pred
