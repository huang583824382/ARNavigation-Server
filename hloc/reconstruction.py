...
def create_empty_db(database_path: Path):
    ...
    db.create_tables()
    cam_id = 1
    cam_tuple = (1, 600, 800, (1232.66914, 1232.96041, 869.374958, 649.306551))
    db.add_camera(*cam_tuple, camera_id=cam_id)
    db.commit()
    db.close()

def add_camera_params(database_path: Path, params = None):
    if database_path.exists():
        logger.warning('The database already exists, deleting it.')
        database_path.unlink()
    logger.info('Adding camera module...')
    db = COLMAPDatabase.connect(database_path)
    cam_id = 0
    cam_tuple = (1, 600, 800, tuple(1232.66914, 1232.96041, 869.374958, 649.306551))
    db.add_camera(*cam_tuple, camera_id=cam_id)
    db.commit()
    db.close()
