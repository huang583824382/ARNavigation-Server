...
def pairs_from_score_matrix(scores: torch.Tensor,
    ...
    for i, j in zip(*np.where(valid)):
        ...
        scores_list.append(scores[i, indices[i, j]].item())
    return pairs, scores_list

def get_pairs_and_scores(query_descriptor, num_matched, db_descriptors):

    name2db = {str(i)+'-'+n: i for i, p in enumerate(db_descriptors)
               for n in list_h5_names(p)}
    # print('name2db', len(name2db))
    # print(name2db)
    ...
    # Avoid self-matching
    self = np.array([''])[:, None] == np.array(db_names)[None]

    pairs, scores = pairs_from_score_matrix(sim, self, 20, min_score=0)
    # db_names中是图片名
    # print(len(db_names), pairs, scores)
    res = {str(db_names[j[1]]): scores[i] for i, j in enumerate(pairs)}
    # print(pairs)
    # print(db_names)
    # print(name2db)
    # logger.info(f'Found {len(pairs)} res')
    # print(res)
    return res
    # return None
...
def main(descriptors, output, num_matched,
    ...
    print('db_names_h5', len(db_names_h5))
    ...
    pairs, scores = pairs_from_score_matrix(sim, self, num_matched, min_score=0)
    # db_names中是图片名
